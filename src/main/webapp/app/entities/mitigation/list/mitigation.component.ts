...
export class MitigationComponent implements OnInit {
  ...
  search(query: string): void {
    if (query && ['mitigationID', 'controlID', 'reference', 'type', 'status'].includes(this.predicate)) {
      this.predicate = 'mitigationID';
      this.ascending = true;
    }
  ...
  protected sort(): string[] {
    ...
    if (this.predicate !== 'mitigationID') {
      result.push('mitigationID');
    }
    return result;
  }
  ...
  protected onSuccess(data: IMitigation[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/mitigation'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
